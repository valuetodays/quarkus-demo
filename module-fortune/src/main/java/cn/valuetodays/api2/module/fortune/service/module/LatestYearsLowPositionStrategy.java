 ...
 *
 * @author lei.liu
 * @since 2023-05-22 08:15
 */
@Slf4j
@ApplicationScoped
public class LatestYearsLowPositionStrategy implements InvestStrategy {
    @Inject
    QuoteDailyStatServiceImpl quoteDailyStatService;
    @Inject
    QuoteStatModule quoteStatModule;
    ...
    public InvestStrategyResp doInvest(InvestStrategyReq req) {
        ...
        for (int i = 0; i < tradeDays.size(); i++) {
            ...
            QuoteStatResp resp = quoteStatModule.computeStatOfLatest3Years(dataList, InvestStrategy.fromYyyyMMdd(statDateInTradeDay));
            double ptgFromHighest = resp.getPtgFromHighest();
            boolean shouldBuy = false;
            // 最高点到现在已跌了30%，就买
            if (ptgFromHighest < -0.3) {
                shouldBuy = true;
            }
